...
class Setup(commands.Cog, name="setup"):
    ...
    async def setup(self, interaction: discord.Interaction, user: discord.User) -> None:
        ...
        eb1 = await text_channel.send(
            ...
            .settitle("Welcome")
            .setdescription(
                "Welcome to GPTHelper. To use the bot, simply say ``start chat`` and the bot will create a thread within the channel. To archive the thread, simply say ``close chat``. The entire thread history is sent each time you ask a question. Keep in mind this will increase the cost of each time you use the bot. \nIf you would like to start fresh, just say the ``restart  chat``. All text is formatted using LATEX.")
            .black()
            .build()
        )
        eb2 = await text_channel.send(
            ...
            .settitle("Settings")
            .setdescription(
                "To modify the bot's settings, use the /settings command. You can modify the temperature, max tokens, and model. The bot will remember your settings for future use.")
            .black()
            .build()
        )
        await eb1.pin()
