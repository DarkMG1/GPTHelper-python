...
class Models(commands.Cog, name="models"):
    ...
    async def models(self, interaction: discord.Interaction) -> None:
        eb = (getbaseembedbuilder()
              ...
              .setdescription(
            "Below are the models this bot currently supports. When wanting to specify a specific model, use the exact model name used below.")
              .black())
        for model in Model:
            eb.addfield(model.model_name(),
                        "Input: $" + str(model.input_cost()) + "\nOutput: $" + str(model.output_cost()), True)
